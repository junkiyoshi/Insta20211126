...
void ofApp::update() {
	...
	for (int x = -150; x <= 150; x += 5) {
		...
		auto noise_value = ofMap(ofNoise(x * 0.04, ofGetFrameNum() * 0.002), 0, 1, 0, 10);
		int noise_value_int = noise_value;
		int base_hue = ofMap(x, -150, 150, 0, 128);
		for (int i = 0; i < 10; i++) {

			if (i < noise_value) {

				color.setHsb(base_hue, 130, 255);
				this->setBoxToMesh(this->face, this->frame, glm::vec3(x, -150 + i * 15, 0), 15, 5, 15, color, ofColor(0));
			}
			else {

				color.setHsb((base_hue + 128) % 255, 130, 255);
				this->setBoxToMesh(this->face, this->frame, glm::vec3(x, 150 - (10 - i) * 15, 0), 15, 5, 15, color, ofColor(0));
			}
		}

		color.setHsb((int)ofMap(noise_value - noise_value_int, 1, 0, base_hue, base_hue + 128) % 256, 130, 255);
		auto noise_location = glm::vec3(x, ofMap(noise_value - noise_value_int, 1, 0, -150 + (noise_value_int + 1) * 15, 150 - (10 - noise_value_int + 1) * 15), 0);
		this->setBoxToMesh(this->face, this->frame, noise_location, 15, 5, 15, color, ofColor(128));
	}
}
